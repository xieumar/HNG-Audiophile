...
function EarphonesProducts() {
  const products = useQuery(api.products.getAll); // Fetch all products

  // Filter for earphones
  const earphones = products?.filter(product => product.category === "earphones");

  if (!earphones) {
    return <div>Loading earphones...</div>;
  }

  const desiredOrder = ["yx1-earphones"]; // Assuming only one earphone product for now

  const sortedEarphones = earphones.sort((a, b) => {
    ...
    <div className='bg-(--white-light) lg:py-[150px] py-[100px] lg:px-[100px] md:px-[50px] px-[25px]'>
      <div className="mx-auto max-w-[1110px]">
        <div className="mx-auto lg:max-w-[1110px] flex flex-col gap-20 ">
          {sortedEarphones.map((product, index) => (
            <div
              key={product.slug}
              className={`flex flex-col ${index % 2 !== 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} h-auto md:h-fit items-start md:items-center gap-10 md:gap-30`}
            >
